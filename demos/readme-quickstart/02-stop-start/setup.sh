...
mkdir -p "$WS"
( cd "$WS" && "$R" stop stopstart >/dev/null 2>&1; "$R" delete-session stopstart >/dev/null 2>&1 )
pkill -f "$DIR/../_bin/rysh daemon stopstart" 2>/dev/null
cd /; rm -rf "$WS"; mkdir -p "$WS"; cd "$WS"
"$R" create stopstart -d >/dev/null 2>&1 || { echo "[setup] could not create stopstart" >&2; exit 1; }
sleep 3
echo "[setup] stopstart: created, detached, workspace $WS"
